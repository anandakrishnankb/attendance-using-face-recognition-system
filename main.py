...
def TrackImages():
    ...
    if exists1:
        ...
        cv2.destroyAllWindows()
        window.destroy()
    recognized_students = set()  # To keep track of recognized students
    csvFile = None  # Initialize the csvFile variable
        ...
        for (x, y, w, h) in faces:
            ...
            if (conf < 50):
                ...
                if ID not in recognized_students:
                    ...
                    # Create the attendance Excel sheet if it doesn't exist
                    if not os.path.isfile(attendance_file_path):
                        with open(attendance_file_path, 'w') as csvFile:
                            writer = csv.writer(csvFile)
                            writer.writerow(col_names)
                        csvFile.close()

                    # Append the attendance record to the Excel sheet
                    with open(attendance_file_path, 'a') as csvFile:
                        writer = csv.writer(csvFile)
                        writer.writerow(attendance)
                    csvFile.close()

                    # Add the student to the recognized students set
    ...
    # Display the attendance records in the tkinter window
    ts = time.time()
    date = datetime.datetime.fromtimestamp(ts).strftime('%d-%m-%y')
    attendance_file_path = "Attendance\Attendance_" + date + ".csv"
